...
def scaling(k_fold=1):
    ...
    list_of_train_perc_change_scaled = scaler_min_max.transform(np.array(list_of_train_perc_change).reshape(-1,1))
    list_of_val_perc_change_scaled = scaler_min_max.transform(np.array(list_of_val_perc_change).reshape(-1,1))
    ...
    for idx, x in enumerate(list_of_val_perc_change_scaled):
        ...
        update_query = {"percentage_change_scaled_min_max": x[0]}
        db[input_data_collection].update_one({'_id':curr_id}, {"$set": update_query}, upsert=False)
        break
